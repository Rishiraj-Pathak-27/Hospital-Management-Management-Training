...
"${JAVA_PATH%/bin/java}"/bin/javac --module-path "$JAVAFX_PATH" --add-modules javafx.controls -cp "$MYSQL_DRIVER" Main.java HMS.java JDBConn.java

# Run
echo "Running JavaFX application..."
"$JAVA_PATH" --module-path "$JAVAFX_PATH" --add-modules javafx.controls -cp ".:$MYSQL_DRIVER" Main
